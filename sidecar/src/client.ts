...
import { callMcpTool, type McpSession, validateMcpPaymentRequest } from './mcp.ts'
...
function approvalExpiry(challenge: NormalizedChallenge): number {
  ...
  if (!challenge.expiresAt) return localExpiry
  return Math.min(localExpiry, Date.parse(challenge.expiresAt))
}

function deriveIdempotencyKey(request: PaymentRequestInput, challenge: NormalizedChallenge): string {
...
export class MppPaymentClient {
  ...
  constructor(store: PaymentStore, wallet: MppWallet, governedFetch?: typeof fetch) {
    this.#store = store
    this.#wallet = wallet
    this.#fetch = governedFetch ?? createGovernedFetch(() => this.#store.getPolicy())
  }
  ...
  async prepare(rawInput: unknown): Promise<PaymentPreparation> {
    ...
    try {
      ...
      if (!(error instanceof ChallengeCaptured)) throw error
      const challenge = normalizeChallenge(error.challenge, request.url)
      const policy = this.#store.getPolicy()
      const decision = decidePayment(policy, challenge, this.#store.getBudget(policy))
      ...
      return {
        ...
        payment: {
          ...
          request: { method: request.method, transport: 'http', url: request.url },
  ...
  async pay(approvalToken: unknown): Promise<PaymentResult> {
    if (typeof approvalToken !== 'string' || approvalToken.length < 32 || approvalToken.length > 128) {
      throw new AppError('invalid_approval_token', 'Approval token is invalid.')
    }
    const tokenHash = sha256(approvalToken)
    const pending = this.#pending.get(tokenHash)
    this.#pending.delete(tokenHash)
    if (!pending) {
      throw new AppError('approval_not_found', 'Approval is missing, expired, or already used.', 409)
    }
    if (pending.expiresAtMs <= Date.now()) {
      throw new AppError('approval_expired', 'Approval expired before the payment was signed.', 409)
    }

    const priorReceipt = this.#store.findReceiptByChallenge(pending.challenge.challengeHash)
    if (priorReceipt) {
      throw new AppError('already_paid', `Payment was already settled as ${priorReceipt.id}.`, 409)
    }
    ...
    try {
      const policy = this.#store.getPolicy()
      const decision = decidePayment(policy, pending.challenge, this.#store.getBudget(policy))
      if (decision.kind === 'blocked') {
        throw new AppError('payment_blocked', decision.reason, 403)
      }
      const reservation = this.#store.reserve(pending.challenge, pending.idempotencyKey, policy)
      reservationId = reservation.id
      const credential = await paymentClient.createCredential(
        new Response(null, {
          headers: { 'www-authenticate': Challenge.serialize(pending.wireChallenge) },
          status: 402,
        }),
      )
      if (pending.kind === 'mcp') {
        const outcome = await callMcpTool(
          this.#fetch,
          pending.request,
          Credential.deserialize(credential) as unknown as Record<string, unknown>,
          pending.session,
        )
        if (outcome.kind === 'payment_required') {
          const current = normalizeChallenge(outcome.challenge, pending.request.url)
          ...
          throw new AppError('payment_failed', 'MCP server did not accept the approved payment.', 409)
        }
        if (!outcome.receipt) {
          throw new AppError('missing_receipt', 'Paid MCP result did not include a receipt.', 502)
        }
      ...
      const response = await this.#fetch(pending.request.url, { ...init, headers })
      if (response.status === 402) {
        const currentChallenge = Challenge.fromResponseList(response)[0]
        if (currentChallenge) {
          const current = normalizeChallenge(currentChallenge, pending.request.url)
          if (current.challengeHash !== pending.challenge.challengeHash) {
            throw new AppError(
              'challenge_changed',
              'Payment challenge changed after approval. Review the new challenge before paying.',
      ...
      if (reservationId) this.#store.release(reservationId)
      throw error
    }
  }
  ...
  #prunePending(): void {
    ...
    for (const [tokenHash, pending] of this.#pending) {
      if (pending.expiresAtMs <= now) this.#pending.delete(tokenHash)
    }
  }
}
