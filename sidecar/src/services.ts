...
function parseEndpoint(
	value: unknown
): ServiceCatalogItem["endpoints"][number] | null {
...
function parseService(
	...
	const rawEndpoints = Array.isArray(service.endpoints)
		? service.endpoints
		: [];
...
export class MppServiceCatalog {
	...
	async list(
		force = false
	): Promise<{ cached: boolean; services: ServiceCatalogItem[] }> {
		...
		if (!response.ok) {
			...
			throw new AppError(
				"catalog_unavailable",
				"MPP service catalog is unavailable.",
				502
			);
		}
