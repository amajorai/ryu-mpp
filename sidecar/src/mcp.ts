	...
	const visit = (candidate: unknown, depth: number): void => {
		...
		if (nodes > MAX_JSON_NODES || depth > MAX_JSON_DEPTH) {
			throw new AppError(
				...
				"MCP arguments are too deeply nested or complex."
			);
		}
		...
		for (const [key, item] of Object.entries(
			...
			if (FORBIDDEN_KEYS.has(key)) {
				throw new AppError(
					"invalid_mcp_arguments",
					"MCP arguments contain a forbidden object key."
				);
			}
			visit(item, depth + 1);
		}
...
export function validateMcpPaymentRequest(
	input: unknown
): McpPaymentRequestInput {
	if (!input || typeof input !== "object" || Array.isArray(input)) {
		throw new AppError(
			"invalid_mcp_request",
			"MCP payment request must be an object."
		);
	}
	...
	if (url.hash || url.username || url.password) {
		throw new AppError(
			"invalid_url",
			"MCP server URLs cannot contain fragments or credentials."
		);
	}
	if (
	...
	for (const candidate of candidates) {
		try {
			...
			for (const envelope of envelopes) {
				...
				if (expectedId === undefined || typed.id === expectedId) {
					return typed;
				}
			}
		} catch {
...
async function postMcp(
	...
	if (response.status === 404 && session?.id) {
		throw new AppError(
			"mcp_session_expired",
			"The MCP session expired before payment.",
			409
		);
	}
...
function paymentChallenge(
	...
	return parseChallenge(
		(meta as Record<string, unknown>)[Mcp.paymentRequiredMetaKey]
	);
}
...
function resultReceipt(
	...
	const parsed = Receipt.Schema.safeParse(
		(meta as Record<string, unknown>)[Mcp.receiptMetaKey]
	);
...
async function initialize(
	...
	if (
		...
		throw new AppError(
			...
			"MCP server did not accept initialization.",
			502
		);
	}
	const reportedVersion = (envelope.result as Record<string, unknown>)
		.protocolVersion;
	const protocolVersion =
		typeof reportedVersion === "string" &&
		/^[0-9-]{1,32}$/.test(reportedVersion)
			? reportedVersion
...
export async function callMcpTool(
	...
	if (!envelope) {
		throw new AppError(
			"invalid_mcp_response",
			"MCP tool call returned no JSON-RPC response.",
			502
		);
	}
	const challenge = paymentChallenge(envelope);
	if (challenge) {
		return { challenge, kind: "payment_required", session };
	}
	if (envelope.error) {
		const code =
		...
		throw new AppError(
			"mcp_tool_error",
			`MCP tool call failed with JSON-RPC code ${code}.`,
			409
		);
	}
	if (
