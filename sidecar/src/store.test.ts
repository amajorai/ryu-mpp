...
function challenge(overrides: Partial<NormalizedChallenge> = {}): NormalizedChallenge {
  return {
    amountAtomic: '10000',
    chainId: 42_431,
...
describe('PaymentStore', () => {
  ...
  test('enforces daily caps atomically', () => {
    const store = makeStore()
    store.setPolicy({ ...DEFAULT_POLICY, dailySpendCapAtomic: '10000', maxPerRequestAtomic: '10000' })
    store.reserve(challenge(), 'idem-12345678')
    expect(() =>
      store.reserve(challenge({ challengeHash: 'hash-2', challengeId: 'challenge-2' }), 'idem-87654321'),
  ...
  test('rejects idempotency reuse for another challenge', () => {
    ...
    store.reserve(challenge(), 'idem-12345678')
    expect(() =>
      store.reserve(challenge({ challengeHash: 'hash-2', challengeId: 'challenge-2' }), 'idem-12345678'),
  ...
  test('persists paid-endpoint replay claims across restarts', async () => {
    ...
    try {
      expect(
        await first.mppxStore().tryClaim?.('paid-demo:credential:hash-1', Date.now() + 60_000),
      ).toBe(true)
    } finally {
      first.close()
    }
    ...
    try {
      expect(
        await second.mppxStore().tryClaim?.('paid-demo:credential:hash-1', Date.now() + 60_000),
      ).toBe(false)
    } finally {
      second.close()
