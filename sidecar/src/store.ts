...
function sumAtomic(rows: Array<{ amount_atomic: string }>): bigint {
  let total = 0n
  for (const row of rows) total += BigInt(row.amount_atomic)
  return total
}
...
export class PaymentStore {
  ...
  getPolicy(): PaymentPolicy {
    const row = this.#database
      .query<{ value: string }, []>("SELECT value FROM settings WHERE key = 'payment_policy'")
      .get()
  ...
  reserve(
    ...
    const transaction = this.#database.transaction((): Reservation => {
      ...
      if (priorReceipt) {
        throw new AppError('already_paid', 'This payment challenge already has a receipt.', 409)
      }
      ...
      if (existing) {
        ...
        if (existing.status === 'pending') return { budget: this.getBudget(policy), id: existing.id }
        throw new AppError('payment_not_retryable', 'This payment attempt cannot be retried.', 409)
      }

      const budget = this.getBudget(policy)
      if (BigInt(challenge.amountAtomic) > BigInt(budget.availableAtomic)) {
        throw new AppError('daily_budget_exceeded', 'Payment exceeds the remaining daily budget.', 409)
      }
  ...
  commit(
    ...
    challenge: NormalizedChallenge,
    receipt: Omit<ReceiptProjection, 'amountAtomic' | 'chainId' | 'challengeId' | 'currency' | 'id' | 'origin' | 'payee'>,
  ): ReceiptProjection {
    const transaction = this.#database.transaction((): ReceiptProjection => {
      ...
      if (reservation?.status !== 'pending') {
        throw new AppError('reservation_missing', 'Payment reservation is no longer active.', 409)
      }
      if (reservation.challenge_hash !== challenge.challengeHash) {
        throw new AppError('challenge_changed', 'Payment challenge changed after approval.', 409)
      }
      ...
      this.#database
        .query("UPDATE payment_reservations SET status = 'settled', updated_at = ? WHERE id = ?")
  ...
  release(reservationId: string): void {
    this.#database
      .query("UPDATE payment_reservations SET status = 'failed', updated_at = ? WHERE id = ? AND status = 'pending'")
      .run(new Date().toISOString(), reservationId)
  }
  ...
  mppxStore(): Store.AtomicStore<Record<string, unknown>> {
    ...
    return {
      ...
      async get(key) {
        const row = database
          .query<KeyValueRow, [string]>('SELECT value FROM mppx_kv WHERE key = ?')
          .get(key)
