...
import { decidePayment, DEFAULT_POLICY, validatePolicy } from './policy.ts'
...
describe('payment policy', () => {
  test('requires approval by default', () => {
    expect(decidePayment(DEFAULT_POLICY, challenge, budget).kind).toBe('approval_required')
  })

  test('blocks origins outside the allowlist', () => {
    expect(
      decidePayment(DEFAULT_POLICY, { ...challenge, origin: 'https://example.com' }, budget),
