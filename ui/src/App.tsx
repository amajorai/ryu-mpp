...
function initialTab(): Tab {
	const requested =
		typeof window === "undefined" ? undefined : window.ryu?.context?.tab;
	return requested === "policy" || requested === "receipts"
		? requested
		: "services";
}
...
function ReceiptDetail({ receipt }: { receipt: Receipt }): ReactNode {
	return (
		<aside aria-label="Receipt detail" className="receipt-detail">
			...
			<p className="muted">{formatDateTime(receipt.timestamp)}</p>
